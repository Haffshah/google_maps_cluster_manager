...
class MinDistCluster<T extends ClusterItem> {
  final Cluster<T> cluster;
  final double dist;

  MinDistCluster(this.cluster, this.dist);
}
...
class MaxDistClustering<T extends ClusterItem> {
  ...
  List<Cluster<T>> run(List<T> dataset, int zoomLevel) {
    ...
    while (changed) {
      ...
      for (Cluster<T> c in _cluster) {
        MinDistCluster<T>? minDistCluster = getClosestCluster(c, zoomLevel);
  ...
  MinDistCluster<T>? getClosestCluster(Cluster cluster, int zoomLevel) {
    double minDist = 1000000000;
    Cluster<T> minDistCluster = Cluster.fromItems([]);
    for (Cluster<T> c in _cluster) {
      if (c.location == cluster.location) continue;
      double tmp = distUtils.getLatLonDist(c.location, cluster.location, zoomLevel);
      if (tmp < minDist) {
        minDist = tmp;
        minDistCluster = Cluster<T>.fromItems(c.items);
      }
    }
    return MinDistCluster(minDistCluster, minDist);
  }
}
