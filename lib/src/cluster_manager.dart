...
class ClusterManager<T extends clu.ClusterItem> {
  ClusterManager(
    this._items,
    this.updateMarkers, {
    Future<Marker> Function(clu.Cluster<T>)? markerBuilder,
  ...
  final Future<Marker> Function(clu.Cluster<T>) markerBuilder;
  ...
  void _updateClusters() async {
    List<clu.Cluster<T>> mapMarkers = await getMarkers();
  ...
  /// Add on cluster item
  void addItem(clu.ClusterItem newItem) {
  ...
  Future<List<clu.Cluster<T>>> getMarkers() async {
    if (_mapId == null) return List.empty();

    final LatLngBounds mapBounds = await GoogleMapsFlutterPlatform.instance.getVisibleRegion(
      mapId: _mapId!,
    );
    ...
    if (stopClusteringZoom != null && _zoom >= stopClusteringZoom!) {
      return visibleItems
          .map(
            (i) => clu.Cluster<T>.fromItems([i]),
          )
          .toList();
    }

    List<clu.Cluster<T>> markers;
  ...
  List<clu.Cluster<T>> _computeClustersWithMaxDist(List<T> inputItems, double zoom) {
  ...
  List<clu.Cluster<T>> _computeClusters(List<T> inputItems, List<clu.Cluster<T>> markerItems, {int level = 5}) {
    ...
    markerItems.add(clu.Cluster<T>.fromItems(items));
  ...
  static Future<Marker> Function(clu.Cluster) get _basicMarkerBuilder => (cluster) async {
        return Marker(
  ...
  static Future<BitmapDescriptor> _getBasicClusterBitmap(int size, {String? text}) async {
    ...
    return BitmapDescriptor.bytes(data.buffer.asUint8List(), height: 40, width: 40);
  }
}
